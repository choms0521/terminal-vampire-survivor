...
3. `balance.toml`에 진화 1종 추가 (예: `[evolution.unholy_vespers]`)
...
1. `balance.toml`에 보스 정의 추가 (`[enemies.boss_bat]` — HP·속도·특수 공격 패턴; 스폰 시각은 `[director].boss_spawn_times`)
2. `sim/spawn.py` director에 보스 스폰 조건 추가 (시간 임계값, `balance.toml`의 `[director].boss_spawn_times` 참조)
...
python bench/render_spike.py --entity-count <Phase0_cap + boss_overhead>
...
| `config/balance.toml` | 추가 무기·적·진화·영구 업그레이드 데이터, `[director].boss_spawn_times`(보스 스폰 시각) |
| `config/tuning.toml` | `sound_enabled`, 재측정 후 엔티티 캡 갱신 |
...
1. `bench/render_spike.py`로 보스 등장 시 예상 엔티티 수(일반 적 + 보스 투사체)로 부하 측정.
...
def spawn_enemies(state, params: SpawnParams, world, rng) -> None:
    ...
    _spawn_regular(state, params, world, rng)                 # 기존 Phase 2 일반 적
    if params.boss_due and not _boss_alive(state):
        state.enemies.append(_make_boss(world, rng))          # 제자리 추가(sim 내부)
```
...
| 위험 | 가능성 | 영향 | 완화 |
|------|--------|------|------|
| 보스 등장으로 엔티티 수 증가 → Phase 0 엔티티 캡 초과, TPS 저하 | 중~높음 | 높음 | 보스 항목 착수 시 `bench/render_spike.py`로 재측정 필수. `tuning.toml`의 `boss_entity_limit` 조정 후 재검증. 성능 수치는 이 문서에 하드코딩하지 않는다. |
...
| `config/balance.toml` | 추가 무기·적·진화·영구 업그레이드 데이터, `[director].boss_spawn_times`(보스 스폰 시각) |
| `config/tuning.toml` | `sound_enabled`, 재측정 후 엔티티 캡 |
